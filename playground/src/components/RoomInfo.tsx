            ...
            }
            {
                connectedUsers.map(
                    userData => ownUserData?.id !== userData.peer.id &&
                        <div className="flex items-center group relative w-36" key={userData.peer.id}>
                            ...
                            <div className="absolute right-0 hidden group-hover:block">
                                {
                                    followedUser === userData.peer.id ?
                                        <UnfollowButton onClick={() => setFollowedUser(undefined)} /> :
                                        <FollowButton onClick={() => setFollowedUser(userData.peer.id)} />
                                }
                            </div>
                        </div>
                )
...
function FollowButton({ onClick }: ButtonProps) {
    return <div className="cursor-pointer ml-2" onClick={onClick}>
...
function UnfollowButton({ onClick }: ButtonProps) {
    return <div className="cursor-pointer ml-2" onClick={onClick}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
